...
class JioSaavnProvider implements MusicProvider {
  ...
  List _extractList(dynamic data) {
    ...
    if (data is Map) {
      ...
      if (data['data']?['songs'] is List) return data['data']['songs'];
      if (data['data'] is List) return data['data'];
      if (data['results'] is List) return data['results'];
      if (data['songs'] is List) return data['songs'];
    }
  ...
  Future<List<Song>> getAlbumSongs(String albumId) async {
    try {
      ...
      var response = await _dio.get('$_baseUrl/api/albums?id=$cleanId&limit=100').catchError((_) => Response(requestOptions: RequestOptions()));
      var data = response.data?['data'] ?? response.data;
      var rawSongs = _extractList(data);

      if (rawSongs.isEmpty) {
        response = await _dio.get('$_baseUrl/api/albums/$cleanId?limit=100').catchError((_) => Response(requestOptions: RequestOptions()));
        data = response.data?['data'] ?? response.data;
        rawSongs = _extractList(data);
      }
  ...
  Future<List<Song>> getPlaylistSongs(String playlistId) async {
    try {
      ...
      var response = await _dio.get('$_baseUrl/api/playlists?id=$cleanId&limit=100').catchError((_) => Response(requestOptions: RequestOptions()));
      var data = response.data?['data'] ?? response.data;
      var rawSongs = _extractList(data);

      if (rawSongs.isEmpty) {
        response = await _dio.get('$_baseUrl/api/playlists/$cleanId?limit=100').catchError((_) => Response(requestOptions: RequestOptions()));
        data = response.data?['data'] ?? response.data;
        rawSongs = _extractList(data);
      }
  ...
  Future<List<Song>> getArtistTopSongs(String artistId, {String? artistName}) async {
    try {
      ...
      var response = await _dio.get('$_baseUrl/api/artists?id=$cleanId&limit=50').catchError((_) => Response(requestOptions: RequestOptions()));
      var data = response.data?['data'] ?? response.data;
      List rawSongs = _extractList(data);

      if (rawSongs.isEmpty) {
        response = await _dio.get('$_baseUrl/api/artists/$cleanId/songs?limit=50').catchError((_) => Response(requestOptions: RequestOptions()));
        data = response.data?['data'] ?? response.data;
        rawSongs = _extractList(data);
      }

      if (rawSongs.isEmpty && artistName != null && artistName.isNotEmpty) {
        final searchRes = await _dio.get('$_baseUrl/api/search/songs', queryParameters: {'query': artistName, 'limit': 40});
        final searchData = searchRes.data?['data'] ?? searchRes.data;
        rawSongs = _extractList(searchData);
      }
